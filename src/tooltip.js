...
import { isArray, isFunction, isString, isValue, sanitise } from "./util.js";
  ...
  for (i = 0; i < d.length; i++) {
    ...
    if (value !== undefined) {
      text += "<tr class='" +
        ...
        "'>";
      text += "<td class='name'><span style='background-color:" +
        bgcolor +
        "'></span>" +
        name +
        "</td>";
...
ChartInternal.prototype.tooltipPosition = function (
  ...
  if (forArc) {
    tooltipLeft = ($$.width - ($$.isLegendRight
          ? $$.getLegendWidth()
          : 0)) / 2 + mouse[0];
    tooltipTop = ($$.hasType("gauge") ? $$.height : $$.height / 2) + mouse[1] +
      20;
  } else {
    ...
    if (config.axis_rotated) {
      ...
      tooltipLeft = svgLeft + $$.getCurrentPaddingLeft(true) +
        $$.x(dataToShow[0].x) + 20;
      tooltipRight = tooltipLeft + tWidth;
...
ChartInternal.prototype.showTooltip = function (selectedData, element) {
  ...
  var forArc = $$.hasArcType(),
    ...
    positionFunction = config.tooltip_position ||
