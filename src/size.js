...
import { ceil10, isDefined, isValue } from "./util.js";
...
ChartInternal.prototype.getSvgLeft = function (withoutRecompute) {
  var $$ = this,
    config = $$.config,
    hasLeftAxisRect = config.axis_rotated ||
      (!config.axis_rotated && !config.axis_y_inner),
    leftAxisClass = config.axis_rotated ? CLASS.axisX : CLASS.axisY,
    leftAxis = $$.main.select("." + leftAxisClass).node(),
    svgRect = leftAxis && hasLeftAxisRect
...
ChartInternal.prototype.getHorizontalAxisHeight = function (
  ...
  var $$ = this,
    ...
    h = 30;
  if (
    axisId === "x" && !(isDefined(isSubchart) && isSubchart
